  ...
  <div class="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-6">
    ...
    <div class="w-full max-w-2xl bg-white rounded-lg shadow-lg p-6">
      ...
      <ul v-if="!isLoading && cars.length > 0" class="space-y-4">
        <li 
          v-for="company in cars" 
          :key="company.id" 
          class="p-4 bg-gray-100 rounded-lg shadow-md"
        >
          <div :key="index" class="flex gap-4 flex-wrap justify-center">
            <img
          ...
          <div class="grid grid-cols-2">
            <div class="text-sm text-gray-500">Car Type: {{ company.tags.car_type }}</div>
            <div class="text-sm text-gray-500">Company: {{ company.tags.company }}</div>
          </div>
          <div class="text-sm text-gray-500">
            Dealer: {{ company.tags.dealer }}
          </div>
          <div class="flex justify-between mt-4">
            <button 
              class="px-4 py-2 text-red-500 hover:text-red-700 transition duration-300" 
              @click.prevent="deleteCar(company._id)"
            >
              Delete Car
            </button>
            <button 
              class="px-4 py-2 text-orange-400 hover:text-orange-600 transition duration-300" 
              @click.prevent="$router.push(`/car/${company._id}`)"
            >
              Edit Car
            </button>
          </div>
...
export default {
  ...
  methods: {
    deleteCar(carId) {
      // Call Vuex action to delete the car
