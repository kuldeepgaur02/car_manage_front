<template>
    <div class="min-h-screen bg-gray-100 flex items-center justify-center">
      <div class="w-full max-w-2xl bg-white rounded-lg shadow-lg p-6">
        <h2 class="text-2xl font-bold text-center text-gray-700 mb-6">Edit Car</h2>
  ...
  export default {
    ...
    methods: {
      ...
      submitCarForm() {
        this.$store.dispatch('updateCar', { id: this.$route.params.id, car: this.car });
        this.$router.push('/'); // Navigate to home or list page after successful submission
      },
      async fetchCarDetails() {
        const carId = this.$route.params.id;
        console.log(this.$store.state.cars)
        this.car = this.$store.state.cars.filter(car => car._id == carId)[0]; // Fetch car details by id
        this.car.images=[]
        const car = this.$store.state.carDetails;
        if (car) {
          this.car = car;
        } else {
